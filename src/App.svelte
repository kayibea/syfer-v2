<script lang="ts">
  ...
  import { getKeyFromPassphrase } from 'utils/encryption';
  ...
  function start(): void {
    ...
    if (settings.passphrase.length < 1)
      return window.alert('Passphrase cannot be empty. Please enter a valid passphrase.');
  ...
  function clearQueue() {
    if (fileQueue.length > 0 && window.confirm('Clear the file queue? This will cancel all ongoing operations.'))
      fileQueue = [];
  }
</script>
